...
class FeatureCalculator:
    ...
    # NEW METHOD: Get official daily opening price
    def get_daily_open_price(self) -> float:
        ...
        try:
            ...
            daily_open = float(self.daily['open'].iloc[-1])
            
            # DEBUG: Print what opening price we're using
            #print(f"      🔍 Using daily opening price: {daily_open}")
    ...
    def return_since_open(self) -> float:
        """
        FIXED: Calculate return since market open using OFFICIAL daily opening price.
        This is the key fix for the opening price issue.
        """
        try:
            # CRITICAL FIX: Use official daily opening price, NOT intraday minute opens
            official_open_price = self.get_daily_open_price()
            current_price = self.get_current_price()
            
            # DEBUG: Print the calculation details
            #print(f"      📊 Return calc: current={current_price:.2f}, open={official_open_price:.2f}")
            
            if official_open_price <= 0 or current_price <= 0:
                logger.warning(f"Invalid prices: open={official_open_price}, current={current_price}")
                return 0.0
            
            # Calculate return using official daily open
            return_since_open = self.safe_percentage(current_price, official_open_price)
            
            # DEBUG: Print the result
            #print(f"      📈 Return result: {return_since_open:.3f}%")
            
            # Apply reasonable bounds for intraday movement
            return float(np.clip(return_since_open, -20.0, 20.0))  # ±20% max intraday
        except Exception as e:
            logger.error(f"Error calculating return_since_open: {e}")
            return 0.0
    ...
    def volatility_percentile_100(self) -> float:
        """Fixed version - works with limited data."""
        if self.intra.empty:
            return 50.0
            
        try:
            # FIXED: Adaptive data requirements
            available_data = len(self.intra)
            min_required = 20  # Reduced from 100
            target_lookback = min(100, available_data)
            
            if available_data < min_required:
                return 50.0
            
            # FIXED: Use available data, don't force 130
            data = self.intra[['high_1min', 'low_1min', 'close_1min']].dropna()
            
            if len(data) < min_required:
                return 50.0
            
            # Calculate true range (same as before)
            data = data.copy()
            data['hl'] = data['high_1min'] - data['low_1min']
            data['hc'] = abs(data['high_1min'] - data['close_1min'].shift(1))
            data['lc'] = abs(data['low_1min'] - data['close_1min'].shift(1))
            ...
            # FIXED: Adaptive ATR span
            atr_span = min(14, len(data) // 3)
            data['atr'] = data['true_range'].ewm(span=atr_span, adjust=False).mean()
            ...
            # FIXED: Use available periods, don't force 100
            lookback_periods = min(target_lookback, len(atr_values))
            atr_window = atr_values.tail(lookback_periods)
            
            current_atr = atr_window.iloc[-1]
            
            # FIXED: Handle edge case
            if atr_window.nunique() <= 1:
                return 50.0
            
            percentile = (atr_window <= current_atr).sum() / len(atr_window) * 100
    ...
    def position_in_day_range(self) -> float:
        """
        FIXED: Calculate position of price within today's range using proper daily high/low.
        This was potentially corrupted if using minute-level opens for day calculations.
        """
        try:
            # Use intraday data to get actual trading highs and lows (these are correct)
            if self.intra.empty:
                return 0.5
                
            # Get actual day's high and low from minute data (this is correct)
    ...
    def validate_feature_consistency(self, features: Dict[str, float]) -> Dict[str, str]:
        ...
        try:
            # 1. Validate opening price consistency (NEW VALIDATION)
            current_price = self.get_current_price()
            daily_open = self.get_daily_open_price()
            return_since_open = features.get('Return_Since_Open', 0)
            
            if current_price > 0 and daily_open > 0 and abs(return_since_open) < 50:  # Only check reasonable returns
                expected_return = ((current_price - daily_open) / daily_open) * 100
                if abs(expected_return - return_since_open) > 0.1:  # 0.1% tolerance
                    warnings['Return_Since_Open_Consistency'] = f"Expected {expected_return:.3f}%, got {return_since_open:.3f}%"
            
            # 2. Validate VWAP consistency
            ...
            # 3. Validate Bollinger Bands consistency
            ...
            # 4. Validate Order Book Imbalances consistency
            ...
            # 5. Validate spread percentile reasonableness
            ...
            # 6. Validate ATR and volatility consistency
            ...
            # 7. Validate price position bounds
